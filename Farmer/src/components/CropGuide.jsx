...
//   }
//   if(!guide){
//     return <p className="text-center text-red-500">No guide available for {crop} in {location}</p>;
//   }
//   // const cropData=guide;
...
//         {Object.entries(currentCrop.practices).map(([key, practice], index) => {
    ...
    <div className="space-y-8 p-4 md:p-8 animate-grow font-sans">
      ...
      <div className={`${getCardClasses()} p-8 bg-gradient-to-br from-lime-50 to-emerald-100 border-green-200`}>
        <div className="grid lg:grid-cols-2 gap-8 items-center">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <h2 className="text-4xl font-bold text-gray-800">{guide.name}</h2>
              <div className={getBadgeClasses(true)}>
                {guide.difficulty}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="flex items-center gap-2">
                <Calendar className="h-4 w-4 text-green-600" />
                <span className="text-gray-500">Harvest: {guide.timeToHarvest}</span>
              </div>
              <div className="flex items-center gap-2">
                <AlertCircle className="h-4 w-4 text-amber-500" />
                <span className="text-gray-500">Spacing: {guide.spacing}</span>
              </div>
            </div>
          </div>
          <div className="flex justify-center">
      ...
      {/* Best Practices */}
      <div className="grid md:grid-cols-2 gap-6">
        {Object.entries(guide.practices).map(([key, practice], index) => {
          const IconComponent = icons[key];
          return (
            <div key={key} className={`${getCardClasses()} p-6 border-green-200 hover:shadow-xl transition-all duration-300`}>
              <div className="flex items-center gap-3 mb-4">
    ...
    return (
        <div className="bg-gray-50 min-h-screen py-12 px-4 md:px-8">
            <div className="max-w-4xl mx-auto">
                ...
                <CropGuide location={location} crop={selectedCrop} />
            </div>
        </div>
    );
};
